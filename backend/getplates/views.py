...
@getplates_blueprint.route('/api/ml')
def video_feed():
    # Video streaming route. Put this in the src attribute of an img tag

    return Response(camera.runner(), mimetype='multipart/x-mixed-replace; boundary=frame')


@getplates_blueprint.route('/api/ml/stop')
def video_stop():
    camera.status = False
    return jsonify(
        status=False,
...
@getplates_blueprint.route('/api/ml/start')
def video_start():
    camera.status = True
    return jsonify(
        status=True,
